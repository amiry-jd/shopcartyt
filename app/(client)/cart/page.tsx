...
import { getAddresses } from "@/lib/repos/address-repo";
...
const CartPage = () => {
  ...
  const fetchAddresses = async () => {
    ...
    try {
      ...
      const data = await getAddresses(/* TODO userId:*/ "0");
      setAddresses(data);
                    ...
                    {addresses && (
                      <div className="bg-white rounded-md mt-5">
                        <Card>
                          ...
                          <CardContent>
                            <RadioGroup defaultValue={addresses?.find((addr) => addr.default)?.id.toString()}>
                              {addresses?.map((address) => (
                                <div
                                  key={address?.id}
                                  onClick={() => setSelectedAddress(address)}
                                  className={`flex items-center space-x-2 mb-4 cursor-pointer ${
                                    selectedAddress?.id === address?.id && "text-shop_dark_green"
                                  }`}>
                                  <RadioGroupItem value={address?.id.toString()} />
                                  <Label htmlFor={`address-${address?.id}`} className="grid gap-1.5 flex-1">
